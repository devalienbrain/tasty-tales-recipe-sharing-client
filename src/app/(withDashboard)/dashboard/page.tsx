...
import EditProfileModal from "@/components/dashboardComponents/EditProfileModal";
    ...
    <div className="flex flex-col items-center justify-start min-h-screen text-white">
      <div className="w-full max-w-2xl text-center transition duration-300 hover:shadow-2xl">
        ...
        <div className="space-y-4 text-left text-base text-slate-300">
          <p><strong className="font-bold">Name:</strong> {user?.name}</p>
          <p><strong className="font-bold">Role:</strong> {user?.role}</p>
          <p><strong className="font-bold">Email:</strong> {user?.email}</p>
          <p><strong className="font-bold">Phone:</strong> {user?.phone}</p>
          <p><strong className="font-bold">Address:</strong> {user?.address}</p>
          <p><strong className="font-bold">User ID:</strong> {user?._id}</p>
        </div>

        {/* Button for Action */}
        <button
          onClick={handleEditClick}
          className="mt-8 flex items-center justify-center bg-blue-500 text-white px-6 py-3 rounded-full shadow-lg hover:bg-blue-600 transition duration-300"
        >
